...
const NotificationsSection = () => {
  return (
    <div className="bg-white border border-[#1F293726] rounded-2xl p-6 w-full h-full font-['Tajawal'] flex flex-col" dir="rtl">

      <div className="flex justify-between items-center mb-6">
        <h3 className="text-[18px] font-medium text-[#1F2937]">الإشعارات الأخيرة</h3>
      ...
      <div className="flex-1 space-y-4">
        {notifications.map((notif) => (
          <div
            key={notif.id}
            className="w-81.25 h-18 -mr-3 flex items-center gap-2 p-4 border border-[#1F29371A] rounded-lg relative overflow-hidden"
          >
            ...
            <div className="text-right">
              <p className="font-['IBM_Plex_Sans_Arabic'] font-normal mb-2 text-[14px] leading-4 text-[#1F2937]">
                {notif.title}
              </p>
              <p className="font-['IBM_Plex_Sans_Arabic'] font-normal text-[12px] leading-4 text-[#8C9198] mt-1">
                {notif.time}
              </p>
            </div>
          </div>
