...
const RecordingCard = ({ title, subject, date, size, thumbnail, isLive, onPlay, onDownload, onShare }) => (
  <div className="rounded-2xl border border-[#E5E5E5] overflow-hidden bg-white hover:border-gray-300 transition-all">
    ...
    <div className="relative aspect-video bg-gray-100 overflow-hidden">
      ...
      {/* Live badge */}
      {isLive && (
        <span className="absolute top-2 right-2 bg-[#00A63E] text-white text-[10px] font-bold px-2 py-0.5 rounded-full" style={{ fontFamily: "IBM Plex Sans Arabic" }}>
          الآن
        </span>
      )}
      {/* Play overlay */}
    ...
    <div className="p-4" dir="rtl">
      ...
      <div className="flex items-center justify-between">
        <span className="text-[12px] text-[#575F69] flex items-center gap-1" style={{ fontFamily: "IBM Plex Sans Arabic" }}>
          {size}
        </span>
        <div className="flex items-center gap-1">
          <button
            onClick={onShare}
            className="p-1.5 rounded-lg text-[#1F2937] hover:bg-gray-100 transition-all"
          >
