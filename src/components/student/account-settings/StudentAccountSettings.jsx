...
const StudentPersonalCard = ({ student, countryOptions, loadingCountries, onUpdated }) => {
  const buildForm = () => ({
  ...
  useEffect(() => { setForm(buildForm()); }, [student]);
  ...
  const countryLabel = countryOptions.find((c) => c.id === student.country)?.label || student.country;

  return (
    <form onSubmit={handleSubmit} className="bg-(--white) border border-(--border-light) rounded-2xl shadow-(--shadow) p-6">
      <SectionHeader title="البيانات الشخصية" subtitle="هذا القسم يحتوي على بياناتك الشخصية الأساسية التي تُستخدم في جميع الخدمات الرسمية داخل المنصة." editing={editing} onEditClick={() => setEditing(true)} />
      {!editing ? (
        <ViewGrid>
          <ViewField label="الاسم الكامل" value={student.fullName} />
          <ViewField label="اسم المستخدم" value={student.username} />
          <ViewField label="تاريخ الميلاد" value={formatDateDisplay(student.birthDate)} />
...
const StudentAcademicCard = ({ student, onUpdated }) => {
  const buildForm = () => ({
    ...
    curriculumId: student.curriculum?.id || student.curriculum?._id || student.curriculum || '',
  ...
  useEffect(() => {
    if (!editing || !form.stageId) { setGradeOptions([]); return; }
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-(--white) border border-(--border-light) rounded-2xl shadow-(--shadow) p-6">
      <SectionHeader title="البيانات الأكاديمية" subtitle="هذا القسم يحتوي على بياناتك التعليمية الأساسية، والتي تُستخدم لإدارة حصصك ومتابعة رحلتك التعليمية داخل المنصة." editing={editing} onEditClick={() => setEditing(true)} />
      {!editing ? (
        <ViewGrid>
          <ViewField label="المرحلة الدراسية" value={pickName(student.stage?.name)} />
          <ViewField label="الصف الدراسي" value={pickName(student.grade?.name)} />
          <ViewField label="المنهج الدراسي" value={pickName(student.curriculum?.name)} />
          <ViewField label="اللغة المفضلة" value={langLabel(student.studyLanguage)} />
        </ViewGrid>
      ) : (
        <EditBox>
          <Dropdown
            ...
            onChange={(id) => setForm((prev) => ({ ...prev, curriculumId: id, stageId: '', gradeId: '' }))}
          ...
          <Dropdown
            ...
            onChange={(id) => setForm((prev) => ({ ...prev, stageId: id, gradeId: '' }))}
          ...
          <Dropdown
            ...
            placeholder={form.stageId ? 'اختر الصف الدراسي' : 'اختر المرحلة أولاً'}
          />
          <Dropdown
            label="اللغة المفضلة"
