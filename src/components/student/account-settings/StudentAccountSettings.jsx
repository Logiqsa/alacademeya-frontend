...
const StudentPersonalCard = ({ student }) => {
  const buildForm = () => ({
    ...
    <form onSubmit={handleSubmit} className="bg-(--white) border border-(--border-light) rounded-2xl shadow-(--shadow) p-6">
      ...
      {!editing ? (
        <ViewGrid>
          ...
          <ViewField label="الدولة" value={student.country} />
...
const StudentAcademicCard = ({ student }) => {
  const buildForm = () => ({
    ...
    grade: 'g3',
    curriculum: 'eg',
    preferredLanguage: student.preferredLanguage || 'ar',
  });
  ...
  return (
    <form onSubmit={handleSubmit} className="bg-(--white) border border-(--border-light) rounded-2xl shadow-(--shadow) p-6">
      ...
      {!editing ? (
        <ViewGrid>
          <ViewField label="المرحلة الدراسية" value={student.stage} />
          <ViewField label="الصف الدراسي" value={student.grade} />
          <ViewField label="المنهج الدراسي" value={student.curriculum} />
          <ViewField label="اللغة المفضلة" value={langLabel(student.preferredLanguage)} />
        </ViewGrid>
      ) : (
        <EditBox>
          <Dropdown
          ...
          <Dropdown
            ...
            onChange={(id) => setForm((prev) => ({ ...prev, grade: id }))}
          ...
          <Dropdown
            ...
            onChange={(id) => setForm((prev) => ({ ...prev, preferredLanguage: id }))}
    ...
    <form onSubmit={handleSubmit} className="bg-(--white) border border-(--border-light) rounded-2xl shadow-(--shadow) p-6">
      ...
      {!editing ? (
        <div className="border border-x-4 border-[#123C9180] rounded-xl p-5">
          ...
          <p className="text-xs text-(--text-light)">{STATIC_STUDENT.lastPasswordChange}</p>
        </div>
      ) : (
        <EditBox>
