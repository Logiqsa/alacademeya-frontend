...
export default function ExamPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      ...
      const loadedQuiz = unwrap(await getCourseQuiz(courseId, quizId));
      if (active) { setCourse({ id: courseId }); setQuiz(loadedQuiz); }
  ...
  const submitExam = async () => {
    if (quiz?.attemptsRemaining === 0) {
      toast.error('لا توجد محاولات متبقية لهذا الاختبار');
      return;
    }
    ...
    try {
      ...
      navigate('/exam-result/' + courseId + '?quiz=' + encodeURIComponent(quiz.id || quiz._id));
    } catch (error) {
  ...
  return <StudentLayout><div dir={'rtl'} className={'min-h-screen bg-[#F8FAFC] pb-20'}><div className={'mx-auto w-full max-w-[1000px] px-4 pt-6'}>
    <nav className={'mb-6 flex items-center justify-between text-sm text-[#8B94A0]'}><div className={'flex items-center gap-2'}><Link to={'/learn/' + courseId} className={'font-semibold text-[#123C91]'}>الدورة</Link><ChevronLeft size={14} /><span>{quiz.title}</span></div><button disabled={submitting || quiz.attemptsRemaining === 0} onClick={submitExam} className={'rounded-lg bg-[#123C91] px-5 py-2 text-sm font-bold text-white disabled:opacity-60'}>{submitting ? 'جاري التسليم...' : quiz.attemptsRemaining === 0 ? 'لا توجد محاولات متبقية' : 'تسليم الاختبار'}</button></nav>
