...
export default function ExamResultPage() {
  ...
  if (!result) return <Navigate to={'/learn/' + courseId} replace />;
  const score = result.score || {};
  const canRetry = !result.passed && Number(result.attemptsRemaining) > 0;
  ...
  return <StudentLayout><div dir={'rtl'} className={'min-h-screen bg-[#F8FAFC] pb-20'}><div className={'mx-auto w-full max-w-[900px] space-y-6 px-4 pt-6'}>
    <nav className={'flex items-center text-sm text-[#8B94A0]'}><Link to={'/learn/' + courseId} className={'flex items-center gap-2 font-semibold text-[#123C91]'}>الدورة<ChevronLeft size={14} /></Link><span>نتيجة الاختبار</span></nav>
    <div className={'space-y-3 rounded-2xl border bg-white p-8 text-center shadow-sm'}><div className={'mx-auto flex h-16 w-16 items-center justify-center rounded-full ' + (result.passed ? 'bg-emerald-100 text-emerald-600' : 'bg-red-100 text-red-600')}>{result.passed ? <Trophy size={30} /> : <RotateCcw size={30} />}</div><h2 className={'text-2xl font-extrabold'}>{result.passed ? 'أحسنت، اجتزت الاختبار!' : 'لم تصل إلى درجة النجاح بعد'}</h2><div className={'text-4xl font-extrabold ' + (result.passed ? 'text-emerald-600' : 'text-red-600')}>{Math.round(score.percentage || 0)}%</div><p className={'text-sm text-gray-500'}>أجبت عن {score.correctCount || 0} من {score.totalQuestions || 0} إجابات صحيحة</p>{!result.passed && !canRetry && <p className="text-sm font-bold text-red-700">تم استنفاد محاولات الاختبار.</p>}{(result.passed || canRetry) && <button onClick={() => navigate(result.passed ? '/learn/' + courseId : '/exam/' + courseId + '?quiz=' + encodeURIComponent(quizId))} className={'mx-auto mt-3 rounded-lg bg-[#123C91] px-7 py-2.5 text-sm font-bold text-white'}>{result.passed ? 'متابعة الدورة' : 'إعادة الاختبار'}</button>}</div>
