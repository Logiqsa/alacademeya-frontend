...
import { Search, Eye, ChevronDown, ChevronRight, ChevronLeft, ClipboardList, CheckCircle2, Clock3, Loader2, AlertCircle } from "lucide-react";
import AdminLayout from "../../../components/admin/layout/AdminLayout";

import { getPendingSubscriptionRequests, getAllSubscriptions } from "../../../services/authService";
...
const mapSubscription = (sub) => ({
  ...
  guardian: sub.parent?.user?.fullName || sub.student?.parent?.user?.fullName || "--",
  subjects: (sub.items || [])
...
// ─── Filters ──────────────────────────────────────────────────────────────────
const RequestsFilters = ({ search, onSearchChange, filterStatus, onFilterStatusChange, filterDate, onFilterDateChange }) => (
  ...
  return (
    <div className="grid grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4 mb-6" dir="rtl">
      {stats.map((s) => {
        const Icon = s.icon;
        return (
          <div key={s.label} className="bg-white border border-gray-100 rounded-xl p-4 sm:p-5 flex items-center gap-3 sm:gap-4 shadow-sm hover:shadow-md transition-shadow min-w-0">
  ...
  <div className="bg-white border border-gray-200 rounded-2xl p-4 shadow-sm">
    ...
    <div className="mb-3">
      ...
      <div className="flex flex-wrap gap-1">
        {r.subjects.length > 0 ? r.subjects.map((s) => <SubjectTag key={s} label={s} />) : <span className="text-[12px] text-[#9CA3AF]">--</span>}
      </div>
    </div>

    <div className="flex items-center justify-between pt-2.5 border-t border-gray-100">
      <span className="text-[12px] text-[#575F69]">{r.date}</span>
      <button onClick={onView} className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-[12px] font-medium text-[#123C91] bg-[#EAF4FF] hover:bg-[#d6e6fb] transition-colors">
        <Eye size={14} />
...
const SubscriptionRequestsPage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        const pendingMapped = (pendingRes.data.data || []).map(mapPendingStudent);
  ...
  const filtered = useMemo(() => {
    ...
    if (dateFilter === "newest") list = [...list].sort((a, b) => new Date(b.date) - new Date(a.date));
    if (dateFilter === "oldest") list = [...list].sort((a, b) => new Date(a.date) - new Date(b.date));
    return list;
...
const handleView = (r) => {
  if (r.kind === "pending") {
    ...
    navigate(`/admin/subscriptions/requests/${r.id}/activate`, { state: { request: r.raw } });
  } else {
    // مسار عرض التفاصيل (تأكد أن هذا المسار موجود أيضاً)
    navigate(`/admin/subscriptions/requests/${r.id}`, { state: { request: r, isDetails: true } });
  }
};
  ...
  return (
    <AdminLayout>
      <div dir="rtl" className="w-full max-w-full p-3 sm:p-4 md:p-6 font-['IBM_Plex_Sans_Arabic'] overflow-x-hidden">
        ...
        <StatsBar total={requests.length} pending={pendingCount} completed={completedCount} />
        ...
        {paged.length === 0 ? (
          ...
          <>
              ...
              <div className="overflow-x-auto">
                <table className="w-full text-right" style={{ minWidth: 680 }}>
                  ...
                  <tbody className="divide-y divide-gray-100">
                    {paged.map((r) => (
                      <tr key={`${r.kind}-${r.id}`} className="hover:bg-gray-50/70 transition-colors">
                        ...
                        <td className="px-5 py-3.5">
                          <div className="flex items-center justify-center gap-1">
                            <button
                              ...
                              title={r.kind === "pending" ? "تفعيل الاشتراك" : "عرض التفاصيل"}
                            >
                              <Eye size={17} />
                            </button>
                          </div>
            ...
            <div className="md:hidden space-y-3">
              {paged.map((r) => (
                <RequestCard key={`${r.kind}-${r.id}`} r={r} onView={() => handleView(r)} />
              ))}
            </div>
          </>
        )}

        <Pagination page={page} total={filtered.length} totalPages={totalPages} displayed={paged.length} onChange={setPage} />
      </div>
    </AdminLayout>
  );
};
