...
import { Archive, CalendarDays, ChevronLeft, ChevronRight, FileCheck2, FilePlus2, History, LoaderCircle, Pencil, Plus, ShieldCheck, Trash2, Upload } from "lucide-react";
...
const LanguageFields = ({ lang, form, readonly, local }) => {
  ...
  if (readonly) return <article dir={isArabic ? "rtl" : "ltr"} className="policy-rich-content min-h-60 rounded-xl border border-[#E1E7EF] bg-white p-5 text-[#344054] shadow-sm sm:p-7" dangerouslySetInnerHTML={{ __html: normalizeRichTextHtml(form.content?.[lang] || "") }} />;
  return <div className="space-y-2"><label className="text-xs font-bold text-[#475467]">العنوان {label}</label><input value={form.title?.[lang] || ""} onChange={(event) => local("title", lang, event.target.value)} placeholder={isArabic ? "عنوان السياسة بالعربية" : "Policy title in English"} dir={isArabic ? "rtl" : "ltr"} className="h-11 w-full rounded-xl border border-[#D7DEE8] px-3 text-sm outline-none focus:border-[#123C91]" /><label className="block pt-2 text-xs font-bold text-[#475467]">المحتوى {label}</label><RichTextEditor value={form.content?.[lang] || ""} onChange={(value) => local("content", lang, value)} direction={isArabic ? "rtl" : "ltr"} language={lang} placeholder={isArabic ? "اكتب محتوى السياسة بالعربية…" : "Write the policy content in English…"} ariaLabel={`محتوى السياسة ${label}`} /></div>;
};
...
function Criteria({ value, onChange, disabled }) {
  ...
  if (disabled) return <details className="mt-5 rounded-xl border border-[#E1E7EF] bg-white"><summary className="cursor-pointer list-none px-4 py-3 font-extrabold text-[#344054] marker:hidden">معايير ومفاتيح نشر الدورات <span className="mr-2 text-xs font-medium text-[#667085]">({value.length})</span></summary><div className="space-y-3 border-t border-[#EEF1F5] p-4">{value.length ? value.map((criterion, index) => <div key={criterion.key || index} className="rounded-xl bg-[#F8FAFC] p-4"><code dir="ltr" className="inline-block rounded bg-[#E8EDF3] px-2 py-1 text-xs text-[#475467]">{criterion.key || `criterion_${index + 1}`}</code><h4 className="mt-3 font-bold text-[#1F2937]">{criterion.title?.ar || criterion.title?.en || "معيار بدون عنوان"}</h4>{(criterion.description?.ar || criterion.description?.en) && <p className="mt-1 text-sm leading-7 text-[#667085]">{criterion.description?.ar || criterion.description?.en}</p>}</div>) : <p className="text-sm text-[#98A2B3]">لا توجد معايير في هذا الإصدار.</p>}</div></details>;
