...
import { approveAdminInstructorWithdrawal, confirmPaidAdminInstructorWithdrawal, getAdminInstructorWithdrawals, rejectAdminInstructorWithdrawal } from "../../../services/APIService";
...
const PAYMENT_METHODS = [
  { value: "instapay", apiValue: "instapay", label: "InstaPay", destinationLabel: "رقم الهاتف أو عنوان الدفع (IPA)", placeholder: "مثال: 01000000000 أو username@instapay" },
  { value: "bank-transfer", apiValue: "bank-transfer", label: "تحويل بنكي", destinationLabel: "رقم الحساب أو IBAN", placeholder: "أدخل رقم الحساب أو IBAN" },
  { value: "vodafone-cash", apiValue: "wallet", label: "Vodafone Cash", destinationLabel: "رقم محفظة Vodafone Cash", placeholder: "مثال: 01000000000" },
  { value: "orange-cash", apiValue: "wallet", label: "Orange Cash", destinationLabel: "رقم محفظة Orange Cash", placeholder: "مثال: 01200000000" },
  { value: "etisalat-cash", apiValue: "wallet", label: "e& Cash", destinationLabel: "رقم محفظة e& Cash", placeholder: "مثال: 01100000000" },
  { value: "we-pay", apiValue: "wallet", label: "WE Pay", destinationLabel: "رقم محفظة WE Pay", placeholder: "مثال: 01500000000" },
  { value: "other-wallet", apiValue: "wallet", label: "محفظة إلكترونية أخرى", destinationLabel: "اسم المحفظة ورقم الهاتف", placeholder: "مثال: اسم المحفظة - 01000000000" },
  { value: "other", apiValue: "other", label: "وسيلة أخرى", destinationLabel: "بيانات التحويل أو الاستلام", placeholder: "اكتب الوسيلة والبيانات التي تم التحويل إليها" },
];
...
export default function AdminWithdrawalsPage() {
  ...
  const [payment, setPayment] = useState({ method: "instapay", destination: "" });
  ...
  const act = async () => {
    ...
    if (kind === "paid" && !payment.destination.trim()) return toast.error("أدخل بيانات الجهة التي تم التحويل إليها");
    if (kind === "paid" && !detail.trim()) return toast.error("أدخل مرجع عملية التحويل");
    const selectedMethod = PAYMENT_METHODS.find((method) => method.value === payment.method) || PAYMENT_METHODS[0];
    const payload = kind === "approve" ? (detail ? { estimatedTransferHours: Number(detail) } : {}) : kind === "reject" ? { rejectionReason: detail.trim() } : { confirm: true, paymentMethod: selectedMethod.apiValue, paymentDestination: `${selectedMethod.label}: ${payment.destination.trim()}`, externalReference: detail.trim() };
    setWorking(id);
    try {
  ...
  const openAction = (item, kind) => { setAction({ item, kind }); setDetail(""); setPayment({ method: "instapay", destination: "" }); };
  ...
  return <AdminLayout><main dir="rtl" className="mx-auto w-full max-w-400 space-y-5 pb-10 text-right font-['IBM_Plex_Sans_Arabic']">
    ...
    {action && <ActionPanel action={action} detail={detail} setDetail={setDetail} payment={payment} setPayment={setPayment} working={working} onConfirm={act} onCancel={() => setAction(null)} />}
    ...
    <div className="overflow-hidden rounded-2xl border border-[#E1E7EF] bg-white shadow-sm">
      ...
      {loading ? <div className="grid min-h-60 place-items-center text-sm text-[#667085]"><span className="flex items-center gap-2"><LoaderCircle className="animate-spin" />جاري تحميل الطلبات...</span></div> : !items.length ? <div className="grid min-h-60 place-items-center px-4 text-center text-sm text-[#98A2B3]">لا توجد طلبات سحب مطابقة.</div> : <>
        ...
        <div className="space-y-3 p-3 md:hidden">{items.map((item) => <WithdrawalCard key={item.id || item._id} item={item} openAction={openAction} onInstructor={setSelectedInstructor} />)}</div>
      </>}
    </div>
...
const ActionPanel = ({ action, detail, setDetail, payment, setPayment, working, onConfirm, onCancel }) => {
  ...
  return <div className="rounded-2xl border border-[#B9CAE5] bg-[#F7FAFF] p-4 shadow-sm sm:p-5"><div className="flex items-center justify-between gap-3"><div><h2 className="font-extrabold text-[#1F2937]">{action.kind === "paid" ? "تأكيد إتمام الدفع" : config.title}</h2><p className="mt-1 text-xs text-[#667085]">المحاضر: {action.item.instructor?.fullName || action.item.instructor?.name || "—"} · المبلغ: {money(action.item)}</p></div><button type="button" onClick={onCancel} className="grid size-9 place-items-center rounded-full text-[#667085] hover:bg-white" aria-label="إلغاء"><XCircle size={20} /></button></div>
    {action.kind === "paid" ? <div className="mt-4 grid gap-4 sm:grid-cols-2">
      <label className="text-xs font-bold text-[#475467]"><span className="mb-1.5 block">طريقة التحويل <b className="text-red-500">*</b></span><select value={payment.method} onChange={(event) => setPayment({ method: event.target.value, destination: "" })} className="h-11 w-full rounded-xl border border-[#D7DEE8] bg-white px-3 text-sm outline-none focus:border-[#123C91]">{PAYMENT_METHODS.map((method) => <option key={method.value} value={method.value}>{method.label}</option>)}</select></label>
      <label className="text-xs font-bold text-[#475467]"><span className="mb-1.5 block">{selectedMethod.destinationLabel} <b className="text-red-500">*</b></span><input type="text" dir="ltr" value={payment.destination} onChange={(event) => setPayment((current) => ({ ...current, destination: event.target.value }))} placeholder={selectedMethod.placeholder} className="h-11 w-full rounded-xl border border-[#D7DEE8] bg-white px-3 text-right text-sm outline-none focus:border-[#123C91]" /></label>
