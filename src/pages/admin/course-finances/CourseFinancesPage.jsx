...
const plainAmount = (value) => Number(value || 0).toLocaleString("en-US", { maximumFractionDigits: 2 });
const date = (value) => {
...
const CourseFinancesPage = () => {
  ...
  const courseOptions = useMemo(() => [...new Map([...filterOptions.courses, ...sections.courses.data, ...sections.ledger.data.items].filter((item) => item.courseId || item.id).map((item) => [String(item.courseId || item.id), { id: item.courseId || item.id, name: item.course, instructorId: item.instructorId }])).values()].filter((item) => !draftFilters.instructorId || String(item.instructorId) === String(draftFilters.instructorId)), [draftFilters.instructorId, filterOptions.courses, sections.courses.data, sections.ledger.data.items]);
  ...
  const changeFilter = (key, value) => setDraftFilters((current) => {
    if (key === "courseId") return { ...current, courseId: value, instructorId: value ? "" : current.instructorId };
    if (key === "instructorId") return { ...current, instructorId: value, courseId: "" };
  ...
  return <AdminLayout><main dir="rtl" className="mx-auto w-full max-w-400 space-y-6 pb-10 text-right font-['IBM_Plex_Sans_Arabic']">
    ...
    <Filters value={draftFilters} courses={courseOptions} instructors={instructorOptions} loading={anyLoading} onChange={changeFilter} onApply={applyFilters} onReset={resetFilters} />
    {!initialized ? <div className="rounded-2xl border bg-white"><LoadingState label="جاري تحميل البيانات المالية..." /></div> : <>
      ...
      <Section section={sections.courses} retry={() => retry("courses")}><AnalyticsTable title="تفاصيل مبيعات الدورات" type="courses" items={sections.courses.data} onInstructor={setSelectedInstructor} /></Section>
      <Section section={sections.instructors} retry={() => retry("instructors")}><AnalyticsTable title="مبيعات المحاضرين" type="instructors" items={sections.instructors.data} onInstructor={setSelectedInstructor} /></Section>
...
const Filters = ({ value, courses, instructors, loading, onChange, onApply, onReset }) => <div className="rounded-2xl border border-[#E1E7EF] bg-white p-4 shadow-sm sm:p-5"><Heading icon={Filter} title="تصفية النتائج" subtitle="بدون تحديد فترة سيتم عرض جميع المبيعات المسجلة" /><div className="mt-4 grid gap-3 sm:grid-cols-2 xl:grid-cols-4"><Field label="من تاريخ"><input type="date" value={value.from} onChange={(e) => onChange("from", e.target.value)} /></Field><Field label="إلى تاريخ"><input type="date" value={value.to} onChange={(e) => onChange("to", e.target.value)} /></Field><SearchableSelect label="الدورة" value={value.courseId} options={courses} placeholder={value.instructorId ? "كل دورات المحاضر" : "كل الدورات"} searchPlaceholder="ابحث عن دورة..." onChange={(selected) => onChange("courseId", selected)} />{!value.courseId && <SearchableSelect label="المحاضر" value={value.instructorId} options={instructors} placeholder="كل المحاضرين" searchPlaceholder="ابحث عن محاضر..." onChange={(selected) => onChange("instructorId", selected)} />}</div><div className="mt-4 flex flex-col-reverse gap-2 sm:flex-row"><button type="button" onClick={onReset} disabled={loading} className="h-11 rounded-xl border px-5 font-bold text-[#475467] disabled:opacity-50">إعادة ضبط</button><button type="button" onClick={onApply} disabled={loading} className="h-11 rounded-xl bg-[#123C91] px-6 font-bold text-white disabled:opacity-50">تطبيق</button></div></div>;
...
const MoneyCard = ({ icon: Icon, label, rows = [], field }) => <div className="min-h-32 rounded-2xl border border-[#E1E7EF] bg-white p-4 shadow-sm"><div className="mb-3 flex items-center gap-2"><span className="grid size-9 place-items-center rounded-lg bg-[#EAF2FF] text-[#123C91]"><Icon size={18} /></span><span className="text-xs font-bold text-[#667085]">{label}</span></div>{rows.length ? <div className="space-y-1.5">{rows.map((row) => <div key={row.currency} dir="ltr" className="flex items-baseline justify-between gap-2"><b className="text-lg font-extrabold tabular-nums text-[#1F2937]">{plainAmount(row[field])}</b><span className="rounded bg-[#F2F4F7] px-2 py-0.5 text-[11px] font-bold text-[#475467]">{row.currency}</span></div>)}</div> : <p className="text-sm text-[#98A2B3]">لا توجد بيانات</p>}</div>;
