...
const EventsTable = ({ items, onSuspend, suspending }) => <div className="overflow-x-auto"><table className="w-full min-w-[1280px] text-right text-xs"><thead className="bg-[#F8FAFC] text-[#667085]"><tr>{["المستخدم", "الدور", "الدورة والدرس", "المورد", "المحاولات", "التصنيف", "أول محاولة", "آخر محاولة", "المتصفح والجهاز", "IP", "سبب الوصول", "الإجراء"].map((heading) => <th key={heading} className="whitespace-nowrap px-4 py-3.5">{heading}</th>)}</tr></thead><tbody className="divide-y divide-[#EEF1F5]">{items.map((item) => { const level = levelOf(item.attemptCount); const trust = trustOf(item); const protectedActor = ["admin", "super-admin"].includes(item.user?.role) || item.accessReason === "instructor"; return <tr key={item.id} className="align-top hover:bg-[#FAFCFF]"><td className="px-4 py-4"><strong className="block text-sm">{item.user?.name || "مستخدم محذوف"}</strong><span dir="ltr">{item.user?.email || "—"}</span></td><td className="px-4 py-4">{trust ? <span className="whitespace-nowrap rounded-full bg-purple-50 px-2.5 py-1 font-semibold text-purple-700 ring-1 ring-purple-100">{trust}</span> : item.actorRoleSnapshot}</td><td className="px-4 py-4"><strong className="block">{titleOf(item.course?.title)}</strong><span className="text-[#667085]">{item.lesson?.title || "—"}</span></td><td className="px-4 py-4">{item.resourceType === "primary" ? "محتوى أساسي" : "مرفق"}</td><td className="px-4 py-4 text-center text-lg font-bold text-[#123C91]">{item.attemptCount}</td><td className="px-4 py-4"><span className={`whitespace-nowrap rounded-full px-2.5 py-1 font-semibold ring-1 ${level[1]}`}>{level[0]}</span></td><td className="whitespace-nowrap px-4 py-4">{dateTime(item.firstOccurredAt)}</td><td className="whitespace-nowrap px-4 py-4">{dateTime(item.lastOccurredAt)}</td><td dir="ltr" className="max-w-56 truncate px-4 py-4 text-right" title={item.userAgentSummary}>{item.userAgentSummary}</td><td dir="ltr" className="px-4 py-4 text-right">{item.ipSummary}</td><td className="px-4 py-4">{item.accessReason}</td><td className="px-4 py-4"><button type="button" disabled={protectedActor || !item.user?.id || item.user.isActive === false || suspending === item.id} title={protectedActor ? "الأدمن ومحاضر الدورة مستثنيان" : "إيقاف الحساب"} onClick={() => onSuspend(item)} className="whitespace-nowrap rounded-lg bg-red-50 px-3 py-2 font-bold text-red-700 disabled:cursor-not-allowed disabled:bg-slate-100 disabled:text-slate-400">{item.user?.isActive === false ? "موقوف بالفعل" : suspending === item.id ? "جارٍ الإيقاف..." : "إيقاف الحساب"}</button></td></tr>; })}</tbody></table></div>;
