...
export default function CoursePlayerPage() {
  ...
  const layoutProps = instructorView
    ? { showBreadcrumbs: false }
    : { marketplaceOnly: learnerOnly, showBreadcrumbs: false };
  const libraryPath = instructorView
    ? (user?.role === 'teacher' ? '/teacher/my-courses' : '/teacher/learning')
    : learnerOnly
      ? '/learner-dashboard'
  ...
  const allLessons = (view?.curriculum || []).flatMap((section) => section.lessons || []);
  const allQuizzes = [
    ...(view?.quizzes || []),
    ...allLessons.flatMap((lesson) => lesson.quizzes || []),
  ];
  const hasExhaustedQuiz = allQuizzes.some((quiz) => quiz.attemptsRemaining === 0);
  const certificateRequirementsSatisfied = allLessons.length > 0
    && allLessons.every((lesson) => lesson.progress?.status === 'completed')
    && allQuizzes.every((quiz) => !quiz.isRequired || quiz.passed || quiz.attemptsRemaining === 0);
  const canClaimCertificate = view?.certificateEligible || certificateRequirementsSatisfied;
  const lessonAlreadyCompleted = currentLesson?.progress?.status === 'completed';
  const canFinalizeCourse = lessonAlreadyCompleted && hasExhaustedQuiz;
  ...
  const completeLesson = async () => {
    ...
    try {
      ...
      if (!lessonAlreadyCompleted && all[index + 1]) setCurrentLesson(all[index + 1]);
      toast.success(updated.progress?.isCompleted ? 'تم إكمال الدورة' : lessonAlreadyCompleted ? 'تم تحديث تقدم الدورة' : 'تم إكمال الدرس');
  ...
  const claimCertificate = async () => {
    setWorking(true);
    try {
      let updated = await load();
      if (!updated.certificateEligible && !updated.certificateIssued && currentLesson?.id) {
        await completeCourseLesson(view.course.id, currentLesson.id);
        updated = await load();
      }
      if (!updated.certificateEligible && !updated.certificateIssued) {
        throw new Error('أكمل جميع الدروس والاختبارات أولاً');
      }
      if (!updated.certificateIssued) await claimCourseCertificate(view.course.id);
      navigate(`/certificate/${view.course.id}`);
    }
    catch (error) { toast.error(error?.response?.data?.message || error?.message || 'تعذر إصدار الشهادة'); }
    finally { setWorking(false); }
  };
  ...
  const isImageLesson = currentMediaType === 'document' && mediaMimeType.toLowerCase().startsWith('image/');
  const displaySections = (view.curriculum || []).filter((section) => section.lessons?.length || section.quizzes?.length);
  ...
  const renderCurriculumLesson = (lesson) => {
    ...
    return <div key={lesson.id} className='border-t border-[#EDF1F5] bg-white'>
      ...
      {!!downloadableAttachments.length && <div className='flex flex-wrap gap-1.5 border-t border-dashed border-[#E3E9F1] bg-[#FAFCFF] px-3 py-2'>
        {downloadableAttachments.map((attachment) => {
          const attachmentId = attachment.id || attachment._id;
          const attachmentName = attachment.originalName || attachment.name || 'ملف الدرس';
          const imageAttachment = isImageAttachment(attachment);
          return <div key={attachmentId} className='inline-flex max-w-full items-center rounded-md border border-[#D7E2F3] bg-white transition hover:border-[#123C91]'>
            <button type='button' onClick={() => openAttachment(attachment, lesson)} disabled={!!openingAttachmentId} title={imageAttachment ? `عرض ${attachmentName}` : `تنزيل ${attachmentName}`} className='inline-flex max-w-48 min-w-0 items-center gap-1.5 px-2 py-1.5 text-right text-[11px] font-semibold text-[#123C91] disabled:cursor-wait disabled:opacity-60'>
              ...
              <span className='min-w-0 truncate' dir='auto'>{attachmentName}</span>
            </button>
            {imageAttachment && <button type='button' onClick={() => downloadAttachment(attachment, lesson)} disabled={!!openingAttachmentId} aria-label={`تنزيل ${attachmentName}`} title={`تنزيل ${attachmentName}`} className='mx-1 grid h-7 w-7 shrink-0 place-items-center rounded border border-[#D7E2F3] bg-white text-[#123C91] transition hover:bg-[#123C91] hover:text-white disabled:opacity-60'><Download size={13} /></button>}
          </div>;
        })}
      </div>}
    </div>;
  ...
  return <Layout {...layoutProps}><div dir='rtl' className='min-h-full rounded-xl bg-[#F4F7FB] p-1 text-[#202936] sm:rounded-2xl sm:p-3 lg:p-4'>
    <div className='mx-auto max-w-[1450px]'><div className='mb-4 overflow-hidden rounded-xl bg-linear-to-l from-[#123C91] via-[#174BAE] to-[#116B91] px-4 py-4 text-white shadow-[0_12px_30px_rgba(18,60,145,0.18)] sm:mb-6 sm:rounded-2xl sm:px-7 sm:py-5'>
      ...
      <div className='flex flex-col gap-4 sm:flex-row sm:flex-wrap sm:items-end sm:justify-between'><div className='min-w-0'><span className='mb-2 block text-xs font-semibold text-[#8EF0E3]'>أنت تتعلم الآن</span><h1 className='truncate text-xl font-extrabold sm:text-2xl'>{titleOf(view.course.title)}</h1></div><div className='grid w-full gap-3 min-[430px]:grid-cols-2 sm:flex sm:w-auto sm:flex-wrap sm:items-center'><div className='min-w-0 rounded-xl bg-white/10 px-4 py-3 backdrop-blur sm:min-w-44'><div className='mb-2 flex justify-between text-xs'><span>تقدم الدورة</span><b dir='ltr'>{Math.round(progress)}%</b></div><div className='h-2 overflow-hidden rounded-full bg-white/20'><div className='h-full rounded-full bg-[#26D6C1] transition-all duration-500' style={{ width: `${progress}%` }} /></div></div>{view.certificateIssued && <Link to={`/certificate/${courseId}`} className='inline-flex h-11 items-center justify-center gap-2 rounded-xl bg-[#12C6B0] px-5 text-sm font-extrabold !text-white shadow-sm transition hover:bg-[#0DAF9B]'><Award size={18} />عرض الشهادة</Link>}</div></div>
    </div>
      <div className={`grid items-start gap-3 ${contentCollapsed ? 'xl:grid-cols-[minmax(0,1fr)]' : 'xl:grid-cols-[minmax(280px,340px)_minmax(0,1fr)]'}`}>
        {!contentCollapsed && <aside className='order-2 overflow-hidden rounded-2xl border border-[#DCE4EF] bg-white shadow-[0_8px_24px_rgba(31,41,55,0.06)] xl:order-1 xl:sticky xl:top-3'><div className='border-b border-[#E8EDF4] bg-[#FAFBFD] px-4 py-3.5'><div className='flex items-center justify-between gap-3'><div className='flex min-w-0 items-center gap-2'><button type='button' onClick={() => setContentCollapsed(true)} className='grid h-8 w-8 shrink-0 place-items-center rounded-lg border border-[#DCE4EF] bg-white text-[#123C91] transition hover:bg-[#EAF2FF]' aria-label='طي محتوى الدورة' title='طي محتوى الدورة'><PanelRightClose size={17} /></button><h2 className='truncate font-extrabold text-[#182338]'>محتوى الدورة</h2></div><b className='rounded-full bg-[#E7F8F5] px-2.5 py-1 text-xs text-[#089E8C]' dir='ltr'>{Math.round(progress)}%</b></div></div>
          <div className='max-h-[calc(100vh-260px)] overflow-y-auto p-3'>{displaySections.map((section, index) => <div key={section.id} className='mb-2 overflow-hidden rounded-xl border border-[#E2E8F0]'><button onClick={() => setOpenSection(openSection === index ? -1 : index)} className='flex w-full items-center justify-between bg-[#F8FAFD] px-4 py-3.5 text-right text-sm font-extrabold text-[#26344B] transition hover:bg-[#EFF4FA]'>{section.title}<ChevronDown size={17} className={`shrink-0 text-[#667085] transition-transform ${openSection === index ? 'rotate-180' : ''}`} /></button>{openSection === index && <>{section.lessons?.map(renderCurriculumLesson)}{section.lessons?.flatMap((lesson) => lesson.quizzes || []).map(renderQuizButton)}{section.quizzes?.map(renderQuizButton)}{index === displaySections.length - 1 && view.quizzes?.map(renderQuizButton)}</>}</div>)}</div>
        </aside>}
        <main className='order-1 min-w-0 xl:order-2'>
          ...
          {!view.certificateIssued && <div className='mt-5 flex flex-col gap-4 rounded-xl border bg-[#F7FAFC] p-4 sm:flex-row sm:items-center sm:justify-between sm:p-5'><div><h2 className='font-extrabold'>شهادة إتمام الدورة</h2><p className='mt-1 text-sm text-gray-500'>{canClaimCertificate ? 'أصبحت مؤهلاً للحصول على الشهادة' : 'أكمل المتطلبات التي يحددها الخادم للحصول عليها'}</p></div><button onClick={claimCertificate} disabled={working || !isLearnerPlayback || !canClaimCertificate} className='flex items-center justify-center gap-2 rounded-lg bg-[#12C6B0] px-5 py-3 font-bold text-white disabled:bg-gray-300'><Award size={18} />إصدار الشهادة</button></div>}
        </main>
      </div>
    </div>
