...
export default function CoursePlayerPage() {
  ...
  return <Layout {...layoutProps}><div dir='rtl' className='min-h-full rounded-2xl bg-[#F4F7FB] p-2 text-[#202936] sm:p-3 lg:p-4'>
    <div className='mx-auto max-w-[1450px]'><div className='mb-6 overflow-hidden rounded-2xl bg-linear-to-l from-[#123C91] via-[#174BAE] to-[#116B91] px-5 py-5 text-white shadow-[0_12px_30px_rgba(18,60,145,0.18)] sm:px-7'>
      ...
      <div className='flex flex-wrap items-end justify-between gap-4'><div><span className='mb-2 block text-xs font-semibold text-[#8EF0E3]'>أنت تتعلم الآن</span><h1 className='text-xl font-extrabold sm:text-2xl'>{titleOf(view.course.title)}</h1></div><div className='flex flex-wrap items-center gap-3'><div className='min-w-44 rounded-xl bg-white/10 px-4 py-3 backdrop-blur'><div className='mb-2 flex justify-between text-xs'><span>تقدم الدورة</span><b dir='ltr'>{Math.round(progress)}%</b></div><div className='h-2 overflow-hidden rounded-full bg-white/20'><div className='h-full rounded-full bg-[#26D6C1] transition-all duration-500' style={{ width: `${progress}%` }} /></div></div>{view.certificateIssued && <Link to={`/certificate/${courseId}`} className='inline-flex h-11 items-center gap-2 rounded-xl bg-[#12C6B0] px-5 text-sm font-extrabold !text-white shadow-sm transition hover:bg-[#0DAF9B]'><Award size={18} />عرض الشهادة</Link>}</div></div>
    </div>
      <div className={`grid items-start gap-3 ${contentCollapsed ? 'xl:grid-cols-[minmax(0,1fr)]' : 'xl:grid-cols-[340px_minmax(0,1fr)]'}`}>
        ...
        <main className='order-1 min-w-0 xl:order-2'>
          ...
          {!view.certificateIssued && <div className='mt-5 flex items-center justify-between rounded-xl border bg-[#F7FAFC] p-5'><div><h2 className='font-extrabold'>شهادة إتمام الدورة</h2><p className='mt-1 text-sm text-gray-500'>{canClaimCertificate ? 'أصبحت مؤهلاً للحصول على الشهادة' : 'أكمل المتطلبات التي يحددها الخادم للحصول عليها'}</p></div><button onClick={claimCertificate} disabled={working || !isLearnerPlayback || !canClaimCertificate} className='flex items-center gap-2 rounded-lg bg-[#12C6B0] px-5 py-3 font-bold text-white disabled:bg-gray-300'><Award size={18} />إصدار الشهادة</button></div>}
        </main>
      </div>
    </div>
