    ...
    <Layout marketplaceOnly={isCourseLearnerDashboard}>
      <div className="mx-auto max-w-7xl p-1 sm:p-4" dir="rtl">
        ...
        <StatusBar stats={{
          ...
          activeCourses: enrolledCourses.filter((course) => !course.progressData.isCompleted && course.enrollmentStatus !== 'revoked').length,
          tests: enrolledCourses.reduce((total, course) => total + course.progressData.completedTests, 0),
        }} />

        {loading ? (
          <div className="mt-6 rounded-xl border bg-white py-16 text-center text-[#7B8490]">جاري تحميل دوراتك...</div>
        ) : enrolledCourses.length > 0 ? (
          <>
            <CoursesFilter onSearch={(value) => { setSearch(value); setPage(1); }} onChange={updateFilters} />
            {paginatedCourses.length > 0 ? (
              <CoursesGrid courses={paginatedCourses} onComplete={setCertificateCourse} />
            ) : (
