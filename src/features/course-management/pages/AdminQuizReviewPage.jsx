...
import { fetchAdminCourse, fetchTeacherCourse } from '../api/coursesApi';

export default function AdminQuizReviewPage({ mode = 'admin' }) {
  const { courseId, lessonId: quizId } = useParams();
  const isTeacher = mode === 'teacher';
  const Layout = isTeacher ? TeacherLayout : AdminLayout;
  const courseBasePath = isTeacher ? '/teacher/courses' : '/admin/courses';
  const [course, setCourse] = useState(null);
  const [loading, setLoading] = useState(true);
  useEffect(() => {
    const fetchCourse = isTeacher ? fetchTeacherCourse : fetchAdminCourse;
    fetchCourse(courseId).then(setCourse).catch(() => setCourse(null)).finally(() => setLoading(false));
  }, [courseId, isTeacher]);
  if (loading) return <Layout><div className={'grid min-h-[50vh] place-items-center'}><LoaderCircle className={'animate-spin text-[#123C91]'} /></div></Layout>;
  const quiz = (course?.quizzes || []).find((item) => String(item._id || item.id) === String(quizId));
  const questions = quiz?.questions || [];
  if (!course || !quiz) return <Layout><div dir={'rtl'} className={'rounded-xl bg-white p-10 text-center'}><CircleHelp className={'mx-auto mb-3 text-[#98A2B3]'} /><p className={'text-[#667085]'}>لم يتم العثور على الاختبار المطلوب.</p><Link to={courseBasePath + '/' + courseId} className={'mt-4 inline-block font-semibold text-[#123C91]'}>العودة إلى تفاصيل الدورة</Link></div></Layout>;
  return <Layout><main dir={'rtl'} className={'min-h-full rounded-xl bg-[#F7F8FC] px-3 py-5 text-right sm:px-5 md:px-7'}>
    <nav className={'mb-8 flex flex-wrap items-center gap-2 text-xs text-[#667085]'}><Link to={courseBasePath} className={'font-semibold text-[#123C91]'}>الدورات</Link><ChevronLeft size={13} /><Link to={courseBasePath + '/' + course.id} className={'font-semibold text-[#123C91]'}>{course.title}</Link><ChevronLeft size={13} /><span>{quiz.title}</span></nav>
