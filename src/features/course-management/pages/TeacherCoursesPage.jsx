...
const TeacherCoursesPage = () => {
  ...
  const handleAction = async (action, courseFromButton = null) => {
    ...
    if (action === "activate") {
      if (selectedCourse.rawStatus !== "archived") return;
      const confirmed = await confirmToast({
        title: "تفعيل الدورة",
        message: `هل تريد تفعيل دورة «${selectedCourse.title}»؟ ستعود للظهور في الرئيسية ويمكن للمتعلمين الجدد الاشتراك بها.`,
        confirmLabel: "تفعيل الدورة",
      });
      if (!confirmed) return;
      const activatingToast = toast.loading("جاري تفعيل الدورة...");
      try {
        await activateTeacherCourse(selectedCourse.id);
        setTeacherCourses((items) =>
          items.map((course) =>
            String(course.id) === String(selectedCourse.id)
              ? { ...course, rawStatus: "published", status: "منشورة" }
              : course,
          ),
        );
        toast.success("تم تفعيل الدورة وظهورها مجددًا", {
          id: activatingToast,
        });
      } catch (error) {
        toast.error(getApiErrorMessage(error, "تعذر تفعيل الدورة"), {
          id: activatingToast,
        });
      }
      return;
