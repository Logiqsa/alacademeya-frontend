...
export default function CoursesGrid({ courses = [], onCancel = null }) {
  ...
  return (
    <div className="mt-6 grid grid-cols-1 gap-4 sm:grid-cols-2 xl:grid-cols-3">
      {courses.map((course) => {
        ...
        const completed = Boolean(course.progressData?.isCompleted);
          ...
          >
            ...
            <div className="p-4 text-right">
              ...
              {course.enrollmentStatus === 'revoked' ? (
                <p className="mt-4 rounded-lg bg-red-50 p-3 text-center text-xs font-bold text-red-700">{course.enrollmentRevocationReason || 'لم يعد التسجيل في هذه الدورة نشطاً.'}</p>
              ) : course.progressData?.isCompleted ? (
                <div className="mt-4 flex gap-2">
                  <Link to={`/courses/${course.slug}#course-reviews`} onClick={(e) => e.stopPropagation()} className="flex h-10 flex-1 items-center justify-center rounded-lg border border-[#DDE4EC] text-xs font-bold text-[#556171]">تقييمات الدورة</Link>
                  <button type="button" onClick={(e) => { e.stopPropagation(); navigate(`/certificate/${course.id}`); }} aria-label="التحقق من حالة الشهادة" className="flex h-10 w-10 items-center justify-center rounded-lg border border-[#DDE4EC] text-[#123C91]"><Award size={15} /></button>
                </div>
              ) : (
                <Link
                  to={`/learn/${course.id}`}
